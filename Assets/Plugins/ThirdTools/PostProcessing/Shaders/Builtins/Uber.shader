Shader "Hidden/PostProcessing/Uber"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile __ RADIAL_BLUR RADIAL_BLUR_FAST
        #pragma multi_compile __ MY_BLOOM
        ...
        // Radial Blur
        TEXTURE2D_SAMPLER2D(_RadialBlurTex, sampler_RadialBlurTex);
        float4 _RadialBlur_Params; // x: center_x, y: center_y, z: intensity, w: not used;

        // MyBloom
        TEXTURE2D_SAMPLER2D(_MyBloomTex, sampler_MyBloomTex);
        float4 _MyBloomTex_TexelSize;
        float _MyBloom_Intensity;
        ...
        half4 FragUber(VaryingsDefault i) : SV_Target
        {
            ...
            #if MY_BLOOM
            {
                half4 bloom = UpsampleBox(TEXTURE2D_PARAM(_MyBloomTex, sampler_MyBloomTex), i.texcoordStereo, _MyBloomTex_TexelSize.xy, 0.5);
                color += bloom * _MyBloom_Intensity;
            }
            #endif

            #if RADIAL_BLUR
            {
                float2 uv1 = 2 * i.texcoordStereo.xy - 1;
                float2 uv2 = 2 * _RadialBlur_Params.xy - 1;
                float dist = length(uv2 - uv1);
                half3 radial_blur = SAMPLE_TEXTURE2D(_RadialBlurTex, sampler_RadialBlurTex, i.texcoordStereo).rgb;
                color.rgb = lerp(color, radial_blur, saturate(_RadialBlur_Params.z * dist));
            }
            #elif RADIAL_BLUR_FAST
            {
                half3 radial_blur = SAMPLE_TEXTURE2D(_RadialBlurTex, sampler_RadialBlurTex, i.texcoordStereo).rgb;
                color.rgb += radial_blur * _RadialBlur_Params.z;
            }
            #endif
