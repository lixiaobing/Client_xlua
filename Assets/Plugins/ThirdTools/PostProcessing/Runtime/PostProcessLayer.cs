...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class PostProcessLayer : MonoBehaviour
    {
        ...
        int RenderBuiltins(PostProcessRenderContext context, bool isFinalPass, int releaseTargetAfterUse = -1, int eye = -1)
        {
            ...
            context.radialBufferNameID = -1;
            context.myBloomBufferNameID = -1;
            ...
            int myDepthOfFieldTarget = RenderEffect<MyDepthOfField>(context, true);
            ...
            int myRadialBlurTarget = RenderEffect<MyRadialBlur>(context);
            ...
            if (!breakBeforeColorGrading)
            {
                ...
                RenderEffect<MyColorGrading>(context);
            }
            ...
            if (myDepthOfFieldTarget > -1) cmd.ReleaseTemporaryRT(myDepthOfFieldTarget);
            if (context.bloomBufferNameID > -1) cmd.ReleaseTemporaryRT(context.bloomBufferNameID);
            if (context.radialBufferNameID > -1) cmd.ReleaseTemporaryRT(context.radialBufferNameID);
            if (context.myBloomBufferNameID > -1) cmd.ReleaseTemporaryRT(context.myBloomBufferNameID);
